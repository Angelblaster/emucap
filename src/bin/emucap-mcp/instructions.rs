...
pub(crate) const SERVER_INSTRUCTIONS: &str = r#"실행 중 레트로 에뮬레이터를 라이브 디버깅한다 — 메모리·상태·화면을 읽고, 메모리/입력을 쓰고, 세이브스테이트·프레임·브레이크포인트를 제어한다. ROM 패치 디버깅용. 대상: Mesen2(SNES·Game Gear), Mednafen 포크(Saturn·PlayStation·PC Engine·Mega Drive/Genesis), Flycast(Dreamcast), MAME PC-98 PoC.
