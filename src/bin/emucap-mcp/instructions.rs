...
[레지스터워치·실행추적] watch_register(register, min, max)=레지스터가 범위를 벗어나는 명령에서 freeze(SP 폭주 등 derail 포착). set_trace(true)+call_stack()/get_trace(count)=호출체인·최근 실행 명령(크래시 직전 경로). call_stack은 호출지 프레임 `[{pc, bank}]`(바깥→안), get_trace는 `[{pc, op, bank?}]`, BP 히트 이벤트도 pc와 함께 `bank`을 싣는다 — `bank`은 pc가 페이징된 ROM 뱅크로 CPU 버스가 좁아 주소만으론 뱅크를 모르는 시스템(Mesen GG/GB)에서만 채워지고, 카트별 가용성은 `status.bank_tagging`이다(SNES는 뱅크가 24비트 pc 안이라 미해당; `bank`이 null/생략이면 그 주소 뱅크 미확정). `.pc`가 유일 보장 필드. break_on_reset=리셋 핸들러 진입 감지. (가용성 status.methods — 없는 플랫폼은 status.capability_notes가 대체[exec BP 콜체인 역추적+step+disasm]를 안내한다.)
